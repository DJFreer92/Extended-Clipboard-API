...
def _row_to_clip(row: Mapping | Sequence[Any]) -> Clip:
    ...
    Updated for extended schema/join queries returning:
    (ClipID, Content, FromAppName, TagsCSV, Timestamp)
    """
    if isinstance(row, (tuple, list)):
        ...
        return Clip(id=clip_id, content=content, from_app_name=from_app, tags=tags_list, timestamp=timestamp)
